...
# yfinance 1m data is limited to 7 days. Intraday (2m-1h) is limited to 60 days.
...
def fetch_and_process_data(tickers_dict, period_days, interval_str, index_ema_span, z_lookback_base, fisher_window):
    try:
        ...
        # Merge and fill gaps to prevent line breakages
...
if df.empty:
    st.warning("⚠️ No data returned. Please adjust settings or wait a moment.")
    st.stop()
...
# Shared line config: connectgaps=True prevents breakages in the visual line
...
def highlight_row(row):
    color = ''
    if row.Signal == 'LONG':
        color = 'background-color: rgba(0, 255, 0, 0.15); color: #00FF00; font-weight: bold'
    elif row.Signal == 'SHORT':
        color = 'background-color: rgba(255, 0, 0, 0.15); color: #FF0000; font-weight: bold'
    return [color if i == len(row)-1 else '' for i in range(len(row))]

# Render robust dataframe
st.dataframe(
    ...
    column_config={
        ...
        "Z_Spread_Diff_Raw": st.column_config.NumberColumn("Z-Spread Raw", format="%.4f"),
        "Z_Spread_14_MA": st.column_config.NumberColumn("Spread 14MA", format="%.4f"),
        "Z_Spread_30_MA": st.column_config.NumberColumn("Spread 30MA", format="%.4f"),
